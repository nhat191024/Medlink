...
class WalletController extends Controller
{
    ...
    public function getRechargeQRCode(Request $request)
    {
        try {
            ...
            $response = $this->paymentService->processAppointmentPayment($data, 'qr_transfer', true);
            ...
            return response()->json([
                ...
                'data' => [
                    'qr_code_url' => $response['data']['checkoutUrl'],
                    'qr_code' => $response['data']['qrCode'],
                    'transaction_id' => $transaction->id,
                    'bill_id' => $response['data']['orderCode'],
                    'amount' => $request->input('amount'),
                ]
            ], Response::HTTP_OK);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json(['error' => 'Failed to create QR code'], Response::HTTP_INTERNAL_SERVER_ERROR);
        }
    }
